    ...
    function odabraniMain(id, odabran) {
        if ($('#'+id).length === 0) {
            if ($(".odabrani").length < 40) {
                ...
                var obrisiOdabranog = "<span class='obrisiOdabranog obrisiOdabranogStyle' title='Please wait..' data-id='"+odabran+"' id='obrisi"+id+"'>"+ odabran +"</span><i class='fas fa-cut scissors_style'></i>";
                ...
                */
                ...
                $('.cokolada_td').each(function(){
                    ...
                    $.ajax({
                        ...
                        success : function(data) {
                            $('#'+id).find('.main_dropdown_menu').append(data);
                        }
                    });
                });
            }
        ...
        $(".obrisiOdabranog").click(function(){
            var v_odabrani = $(this).html();
            $('.main_dropdown_menu_item').each(function(){
                if ($(this).html() == v_odabrani) {
                    $(this).remove();
                }
                
            });
            $('.cokolada_td').each(function(){
                if ($(this).find('.main_dropdown_span').html() == v_odabrani) {
                    $(this).find('.main_dropdown_span').empty();
                }
            });
        ...
        if(isset($search) && !(trim($search)=='') && stripos($row['MAIN'], $search) !== false && !in_array($row['MAIN'], $arr)) {
            ...
            echo '
                <div class="tr">
                    ...
                    <div class="td"><button onclick="odabraniMain(\''.$row['id'].'\', \''.$row['MAIN'].'\');" class="plus"><i class="fas fa-plus fas_style"></i></button></div>
                </div>
            ';
        }
    }
